...
#TODO if BERT:   all_text_features = model.encode_text(text_prompts, attention_masks)
...
def get_top5_tags(score_matrix, tags):
    top5_indices = torch.topk(score_matrix, 10, dim=1).indices  # 상위 5개 인덱스 추출
...
class Zeroshot(Retrieval):
    ...
    def evaluate(self):
        text_prompts = prepare_labels(tags)
